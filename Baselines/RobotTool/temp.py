...
def exec_experiment(fa, cam2, cam3, cam4, cam5, save_dir, Task, ActionList, home_pose, done_queue):    
    ...
    GraspDict = GraspPointDetection(ObjList, ObjLocList[:-1], DescList.values())
...
- The base of the robot arm is at [0.0, 0.0, 0.0]."""
    ...
    with open(save_dir + '/UserPrompt.txt', 'w') as file:
        file.write(UserPrompt)

    analysis = Analyzer(UserPrompt)
    print(analysis)
    with open(save_dir + '/Analysis.txt', 'w') as file:
        file.write(analysis)
